    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        for (k = 1; k < len; k++) {
            ...
            if(grid[rd][cd] != word[k]){
 			...
 			//printf("Found next character for %s: %c at row:%d, col:%d\n", word, grid[rd][cd], rd, cd);
...
int main(int argc, char*argv[]){
    ...
    int length_per_thread = word_count / num_threads; // maybe divisible? think about it
